...
module.exports = opts => {
  ...
  const flatDag = async function * (indexes, parts, max, codec) {
    ...
    if (chunks.length > max) {
      for (let chunk of chunks) {
        ...
        let _indexes = []
        let _parts = []
        for (let { len, block } of flatDag(chunk.indexes, chunk.parts)) {
          yield { len, block }
        ...
        let node = classes.BytesUnion.encoder({ nbl: { indexes: _indexes, parts: _parts }})
  ...
  classes.Data.writer = (opts={}) => {
    ...
    const _end = async () => {
      ...
      if (!indexes.length) return { bytes: Buffer.from('') }
      if (indexes.length === 1) {
        let length = indexes[0][1]
        if (length > opts.inline) {
          return { byteLink: parts[0] }
        } else {
          return { bytes: first.encode() }
        }
      }
      if (indexes.length > opts.maxListLength) {
        let p = parts.map(p => ({ bytes: p }))
        let last
        let results = []
        for await (let result of flatDag(indexes, p, opts.maxListLength)) {
          results.push(result)
        }
        console.log(results)
      } else {
        return { byteLinks: { indexes, parts } }
      }
    }
    return { write: _write, end: _end }
  }
  classes.Data.from = async function * (arr, opts) {
    let { write, end } = classes.Data.writer(opts)
    for (let buffer of arr) {
      yield write(buffer)
    }
    let union = await end()
