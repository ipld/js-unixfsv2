...
module.exports = opts => {
  ...
  const balancedDag = async function * (indexes, parts, max, codec) {
    ...
    parts = [...parts]
    let chunks = []
    let size = Math.floor(indexes.length / max)
    ...
    for (let chunk of chunks) {
      if (chunk.length > max) {
        for (let _chunk of chunks) {
          ...
          let node = classes.BytesUnion.encoder(union)
    ...
    const _end = async () => {
      ...
      if (indexes.length > opts.maxListLength) {
        ...
        if (opts.algorithm === 'balanced') {
          for await (let result of balancedDag(indexes, p, opts.maxListLength)) {
            console.log({result})
            results.push(result)
          }
        } else {
        ...
        let blocks = results.map(r => r.block)
